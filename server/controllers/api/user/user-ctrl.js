...
module.exports.getList = async (req, res, next) => {
  try {
    const searchInfo = req.options

    const result = await handler.getList(searchInfo)
    const total = await handler.getListTotal(searchInfo)
    const query = req.query
    const pagenation = util.makePageData(total,searchInfo.page)
