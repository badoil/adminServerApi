...
module.exports.getList = async (options) => { // condition filter
    ...
    if(srch_orderby){
        switch (srch_orderby) {
            ...
            break
            case "orig_price":
                srch_orderby = 'goods.orig_price'
            break
            case "sales_unit_price":
                srch_orderby = 'goods.sales_unit_price'
            break
            case "pymt_amt":
                srch_orderby = 'goods.pymt_amt'
            break
            case "used_acc_amt":
                srch_orderby = 'goods.used_acc_amt'
            break
            case "used_sp_acc_amt":
                srch_orderby = 'goods.used_sp_acc_amt'
            break
            case "재고있음":
                whereClause += ` AND goods.stock_qty > 0`
                srch_orderby = 'goods.stock_qty'
            break
            case "재고없음":
                whereClause += ` AND goods.stock_qty <= 0`
                srch_orderby = 'goods.stock_qty'
            break
            case "5개이하":
                whereClause += ` AND goods.stock_qty <= 5`
                srch_orderby = 'goods.stock_qty'
            break
            case "10개이하":
                whereClause += ` AND goods.stock_qty <= 10`
                srch_orderby = 'goods.stock_qty'
            break
            case "50개이하":
                whereClause += ` AND goods.stock_qty <= 50`
                srch_orderby = 'goods.stock_qty'
            break
            case "100개이하":
                whereClause += ` AND goods.stock_qty <= 100`
                srch_orderby = 'goods.stock_qty'
            break
        }
    }
    ...
    orderClause += ` ORDER BY ${srch_orderby} ${srch_seq}`
        ...
        let sql = `SELECT goods.*, deal.deliv_amt 
                    FROM (
                        SELECT *
                        FROM goods
                        WHERE 1=1
